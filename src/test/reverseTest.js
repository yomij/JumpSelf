...
function dosUpdate(list, index, bookId, subCallback) {
	async.mapLimit(list, spiderConfig.CONCURRENCY_COUNT, function (cs, callback) {
		cs = Object.assign(cs, {
			bookId,
			_id: bookId + cs.chapterNum
...
module.exports = {
	 ...
	 */
	async  insertFailedBook (bookList) {

		async.mapLimit(bookList, 1, async function (book, callback) {
			try {
				...
				console.log(book.title ? book.title : book + '开始获取')
				return async.mapLimit(list, 1, function (subList, callback) { // 开始分批抓取章节
					dosUpdate(subList, index++, book.bookId, callback)
				}, function (err, result) {
					const failed = []
					const success = []
					result.forEach(item => { // 一本书抓取完成 打印失败内容
					...
					console.log(err,failed.length, JSON.stringify(failed))
					file.writeJson(failed, book.title + ' ' + book.bookId || 'failed'  + ' ' + book.bookId, 'F_PATH')
				})
			} catch (e) {
				console.log('failed', e.message)
				// return spider(book)
